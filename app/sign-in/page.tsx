      ...
      <section className="w-full lg:w-[55%] flex flex-col justify-center px-margin-mobile lg:px-margin-desktop py-stack-lg relative overflow-hidden bg-surface-container-lowest">
        ...
        <div className="w-full max-w-[440px] mx-auto z-10 relative">
          ...
          <div className="text-center mb-stack-lg">
            <h1 className="font-display-lg-mobile md:font-display-lg text-display-lg-mobile md:text-display-lg text-on-surface mb-stack-sm">Welcome back.</h1>
            <p className="font-body-md text-body-md text-on-surface-variant">Sign in to your Bloom workspace.</p>
          </div>
          {/* Google Auth Button */}
          <button className="w-full flex items-center justify-center gap-3 bg-surface-container-lowest border border-[#E8E0D5] rounded-lg py-3 px-4 mb-stack-lg hover:bg-surface-container-low transition-colors shadow-[0_1px_4px_rgba(26,46,46,0.06)] group">
          ...
          {/* Form */}
          <form className="flex flex-col gap-stack-md" method="POST" onSubmit={(e) => { e.preventDefault(); router.push('/dashboard'); }}>
            <div className="flex flex-col gap-2">
              <label className="font-label-sm text-label-sm text-on-surface-variant" htmlFor="email">Email</label>
              <input className="bg-surface-container-lowest border border-[#E8E0D5] rounded-lg px-4 py-3 font-body-md text-body-md text-on-surface focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all placeholder:text-outline-variant" id="email" name="email" placeholder="name@example.com" required type="email" />
            </div>
            <div className="flex flex-col gap-2">
              <div className="flex justify-between items-center">
                <label className="font-label-sm text-label-sm text-on-surface-variant" htmlFor="password">Password</label>
                <Link className="font-label-sm text-label-sm text-primary hover:text-surface-tint transition-colors" href="/auth/forgot-password">Forgot?</Link>
              </div>
              <input className="bg-surface-container-lowest border border-[#E8E0D5] rounded-lg px-4 py-3 font-body-md text-body-md text-on-surface focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all placeholder:text-outline-variant" id="password" name="password" placeholder="••••••••" required type="password" />
            </div>
            <button className="mt-stack-sm w-full bg-primary text-on-primary rounded-lg py-3 px-4 font-label-md text-label-md hover:bg-surface-tint focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary transition-colors" type="submit">
              Sign In
            </button>
          </form>
          <p className="mt-stack-lg text-center font-body-md text-body-md text-on-surface-variant">
            Don&apos;t have an account? <Link className="text-primary hover:text-surface-tint font-medium transition-colors" href="/onboarding">Sign up</Link>
          </p>
        </div>
      </section>
      {/* Right Section (45%) */}
